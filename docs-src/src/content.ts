...
export const pages: Page[] = [
  ...
  },
  {
    ...
    sections: [
      { id: "desktop", title: { en: "Desktop", "zh-TW": "桌面模式" }, body: { en: "Interactive Windows desktops may automatically select Word COM when licensed Word is available. This applies to both DOCX-to-PDF and DOCX-to-HTML conversion; HTML output through Word COM produces higher visual fidelity than semantic extraction.", "zh-TW": "互動式 Windows 桌面在具備合法授權 Word 時，可自動選用 Word COM。此規則同時適用於 DOCX 轉 PDF 與 DOCX 轉 HTML；透過 Word COM 輸出的 HTML 比語意萃取具有更高的視覺忠實度。" } },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "published-image", title: { en: "Published image", "zh-TW": "正式發布映像" }, body: { en: "Tagged releases publish one linux/amd64 image at gordonkit/gordon-doc-converter. The default entrypoint runs the CLI; pass api to start the HTTP service. Prefer an explicit version tag for reproducible deployments.", "zh-TW": "正式版本會發布單一 linux/amd64 映像至 gordonkit/gordon-doc-converter。預設 entrypoint 執行 CLI；傳入 api 則啟動 HTTP service。需要可重現部署時，請使用明確版本 tag。" }, code: "docker run --rm gordonkit/gordon-doc-converter:0.6.0 version\ndocker run --rm -p 8000:8000 -e GORDON_DOC_API_KEY=replace-me gordonkit/gordon-doc-converter:0.6.0 api" },
